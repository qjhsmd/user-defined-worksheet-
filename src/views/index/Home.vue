<template>
    <div class="container">
                        ...
                        <draggable class="components-draggable" :list="item.list"
                            ...
                            <div v-for="(element, index) in item.list" :key="index" class="components-item"
                                ...
                                <div class="components-body">
                                    <svg-icon v-if="element.__config__.tagIcon == 'Customcodetable'" icon-class="user" />
        ...
        >
          复制代码
            ...
            <el-scrollbar class="center-scrollbar">
                <el-row v-if="showTime" class="center-board-row" :gutter="formConf.gutter">
        ...
        <div v-if="rightPanel">
            <right-panel :active-data="activeData" :form-conf="formConf" :drawingList="drawingList"
                :show-field="!!drawingList.length" @tag-change="tagChange" :formData="formData"
                :codeDataOptions="codeDataOptions" @AssembleFormData="AssembleFormData" :codeModelOptions="codeModelOptions" 
                :codeNameOptions="codeNameOptions" :options="fieldName" :clickName="clickName"
                :codeList="codeList" @changeField="changeField" :detailFields="detailFields" :onlyDetail="onlyDetail"
                ref="myRight" />
        </div>
        ...
        <json-drawer size="60%" :visible.sync="jsonDrawerVisible" :json-str="JSON.stringify(formData)"
            @refresh="refreshJson" />
        <code-type-dialog :visible.sync="dialogVisible" :title="$t('home.selectType')" :show-file-name="showFileName"
            @confirm="generate" />
                    ...
                    options.forEach(de => {
                        if (res.__vModel__ === de.value) {
                            if (
                                ...
                                this.rightPanel = false;
                                this.$message.error("date" + this.$t('home.or') + "numeric" + this.$t('home.noSupportTip'));
                                this.$refs.myRight.clean();
                                this.$nextTick(() => {
                                    this.rightPanel = true;
                                });
